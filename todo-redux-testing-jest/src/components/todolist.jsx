...
const TodoList = ({todos,deleteTodoHandler,updateTodoHandler,changeDoneState}) => {
    ...
    if(viewState==="all"){
        content = 
            <div className="flex flex-col items-center w-full">
                {/* all tasks */}
                {complete.length>0 && <h2 className="text-blue-800 font-sans text-3xl font-semibold" data-testid="complete-header">Complete</h2>}
                {complete.map((todo)=>{
                ...
                {incomplete.length>0 && <h2 className="text-blue-800 font-sans text-3xl font-semibold" data-testid="incomplete-header">Incomplete</h2>}
                {incomplete.map((todo)=>{
                    return (<SingleTodo key={todo.id} todo={todo} deleteTodoHandler={deleteTodoHandler} updateTodoHandler={updateTodoHandler} changeDoneState={changeDoneState}/>)
                })}
            </div>
    ...
    return ( 
    <div className='flex flex-col items-center w-full'>
        <select data-testid="filter" name="filter" id="" defaultValue={"all"} onChange={handleFilter} className='mx-64 my-4 border w-32 h-10 p-2'>
