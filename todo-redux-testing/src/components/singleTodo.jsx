...
const SingleTodo = ({todo,deleteTodoHandler,updateTodoHandler,changeDoneState}) => {
    ...
    return ( <div className="w-1/2 flex justify-between my-2 border-gray-400 border p-2 bg-blue-100" key={todo.id} data-test="single-todo">
        {/* show the button or input based on isEditing */}
        {!isEditing && <p className="ml-4">{todo.content}</p>}
        {isEditing && <form onSubmit={submitHandler} className="ml-4" data-test="update-form"><input value={edit} data-test="update-input" onChange={(e)=>{
                e.preventDefault()
                setEdit(e.target.value)
            }} className="border border-gray-400 bg-white h-10 w-64"/></form>}
        <div>
            <button className="bg-blue-400 rounded p-2 mr-2" onClick={editButtonHandler} data-test="update-button">{!isEditing?"update":"undo"}</button>
            <button className="bg-blue-400 rounded p-2 mr-2" onClick={changeStateHandler} data-test={`change-state-button-${todo.done?"undone":"done"}`}>{todo.done?"undone":"done"}</button>
